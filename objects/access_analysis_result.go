...
// between identities, resources, focusing on who can access what and through which
// mechanisms. This evaluates access levels (read/write/admin), access types
...
type AccessAnalysisResult struct {
	...
	// through the analyzed policy configuration. Common examples include Read,
	// Write, List, Delete, Admin, or custom permission levels.
	...
	// access, federated access, or third-party integration access. Examples
	// include 'Cross-Account', 'Public', 'Federated', 'Service-to-Service', etc.
	...
	Accessors []*User `json:"accessors,omitempty"`
	...
	// access can be exercised. These conditions define the circumstances under
	// which the access relationship is valid and the privileges can be used.
	// Examples: IP address restrictions like 'aws:SourceIp:192.0.2.0/24',
	// time-based constraints like 'aws:RequestedRegion:us-east-1', MFA
	// requirements like 'aws:MultiFactorAuthPresent:true', or custom conditions
	// based on resource tags and request context.
	ConditionKeys []*KeyValueObject `json:"condition_keys,omitempty"`

	// The specific privileges, actions, or permissions that are granted through
	// the analyzed access relationship. This includes the actual operations that
	// the accessor can perform on the target resource. Examples: AWS actions like
	// 'sts:AssumeRole', 's3:GetObject', 'ec2:DescribeInstances'; Azure actions
	// like 'Microsoft.Storage/storageAccounts/read'; or GCP permissions like
	// 'storage.objects.get'.
